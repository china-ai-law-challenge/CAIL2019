...
def read_data(config, filepath, data_type, data_filter=None):
    data, shared = prepare_data(filepath)
    print(len(shared["word_counter"])+2)
    ...
    shared_path = config.shared_path or os.path.join(os.path.dirname(filepath), "shared.json")
    if not os.path.exists(shared_path):
        ...
        print("train:   ",len(shared["word2idx"]))
