    ...
    def _build_loss(self):
        ...
        if config.wy:
            ...
            if config.na:
                ...
                losses = tf.nn.softmax_cross_entropy_with_logits_v2(logits=self.concat_logits, labels=tf.cast(concat_y, 'float'))
                concat_y2 = tf.concat(axis=1, values=[na, tf.reshape(self.y2, [-1, M * JX])])
                losses2 = tf.nn.softmax_cross_entropy_with_logits_v2(logits=self.concat_logits2, labels=tf.cast(concat_y2, 'float'))
            else:
                losses = tf.nn.softmax_cross_entropy_with_logits(
